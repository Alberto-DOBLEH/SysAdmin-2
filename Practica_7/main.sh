...
instalar_http_local() {
    echo ""
    echo "Iniciando instalación local de servicios HTTP (desde fuentes oficiales)..."
    if [ -x "./http.sh" ]; then
        ./http.sh
    else
        bash http.sh
...
while true; do
    ...
    echo "2. Instalar servicios HTTP (instalación online)"
    echo "3. Instalar servicios HTTP (descargar .tar.gz desde FTP)"
    echo "4. Salir"
    echo "======================================="
